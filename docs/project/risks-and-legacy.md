...
医院用户列表里 owner/super admin 禁止修改或删除。普通医院用户的角色在修改弹窗里的角色字段维护；如页面仍有独立“角色/分配医院角色”按钮或弹窗，属于重复入口，应收敛移除。个人中心允许超级管理员按手机号转让 owner，成功后强制登出。

默认业务角色使用 `hospital_id=0` 的超级管理员、合伙人、管理员、员工。医院未替换默认角色时继承默认角色权限；修改默认角色权限时，后端复制成当前医院角色并迁移成员；停用或删除默认角色前必须无成员，有成员时拒绝，正确情况下不迁移成员。前端不要展示默认列，也不要让用户直接编辑角色名称和角色层级。
