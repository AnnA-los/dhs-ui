...
7. 平台基础模块保留若依系系统管理、短信配置、系统监控、工具/代码生成、Swagger 等能力。
...
- 短信渠道、短信模板、场景关联和发送日志配置，页面位于 `src/views/system/sms/**`，API 封装位于 `src/api/system/sms.js`。
...
- 短信渠道、模板、场景和发送日志属于系统级配置；数据库保存渠道编码、模板编码，页面展示渠道名称、模板名称，日志手机号和错误详情必须脱敏。

后续扩展注意：

- 短信相关后端菜单 component 需分别指向 `system/sms/channel/index`、`system/sms/template/index`、`system/sms/scene/index`、`system/sms/log/index`。
- 后端接口路径按当前前端封装约定为 `/system/sms/channel`、`/system/sms/template`、`/system/sms/scene-template`、`/system/sms/send-log`；如后端最终调整命名，需同步更新 `src/api/system/sms.js`。
...
- 医院部门、岗位、医院角色、医院用户。
- 邀请码、医院通知。
- 邀请审核、系统消息批次发送、右上角消息统一入口。
- owner 手机号转让。
- 2026-06-27 医院用户前端收敛为手机号必填、邮箱不展示不提交、角色仅在新增/修改弹窗维护；列表展示角色名称，不展示 owner 列和角色层级列。

基线风险：

- 当前医院上下文由 token/后端会话决定，不要在前端业务表单里手传 `hospitalId`。
- 医院 owner 不允许在医院用户列表中编辑、授权、删除；普通用户角色不要再通过独立“分配医院角色”入口维护。
- 医院切换后必须刷新整页。
- 邀请审核通过时前端只提交角色、部门、岗位等稳定 ID；申请人、邀请人、审核人展示名称以后端关联返回为准。
- 系统消息页面只负责创建/发送批次，实际目标解析和投递数量以后端为准。
...
## 2026-06-27 医院组织模块调整记录

- 医院部门页面 `src/views/medical/hospital/dept/index.vue` 调整为树形表格展示，前端按 `deptId`/`parentId` 将接口返回的平铺数据组装为树。
- 医院部门操作列新增“新增子部门”，弹窗默认以上级部门为当前行，同时保留树选择器允许改选。
- 医院部门保存前增加最大 10 层校验，并在编辑父级时排除当前部门及其下级，避免循环父子关系；后端仍需继续做最大层级兜底。
- 需求总结沉淀在 `docs/project/hospital-organization.md`。
