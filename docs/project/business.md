...
### 2026-06-27 任务 7 前端实现记录

- 登录页 `src/views/login.vue` 已收敛为手机号登录，移除邮箱 Tab、邮箱图标和邮箱占位提示；历史 Cookie 中如保存邮箱账号，会清理记住密码 Cookie，不再切换或展示邮箱登录模式。
- 注册页 `src/views/register.vue` 已收敛为手机号注册，移除邮箱 Tab、邮箱输入和邮箱校验规则；提交前防御性删除 `email` 字段，避免继续向 `/register` 发送邮箱资料。
- 登录/注册图形验证码继续受 `/captchaImage` 的 `captchaEnabled` 控制；后端关闭验证码时，前端隐藏验证码区域并清空 `code`、`uuid` 和图片地址。当前前端没有独立短信发送按钮，真实短信关闭时不触发短信发送由后端配置和接口兜底。
- 系统超管右上角由 `Navbar.vue` 统一显示“系统管理后台 / 系统超管”，并在角色/权限未加载完成前不请求医院上下文；识别为系统超管后清空医院态，不调用 `/medical/hospital/current`、`/medical/hospital/my` 和医院通知接口，避免系统后台 404。
...
2026-06-27 任务 1 前端落地摘要：

- `src/views/medical/hospital/user/index.vue` 查询区按用户、部门、岗位、状态筛选，其中用户关键词只表达姓名/手机号。
- 医院用户列表展示用户、手机号、部门、岗位、角色名称、邀请人、状态、创建时间和操作；不再展示 owner 列和角色层级列。
- 新增/修改弹窗移除邮箱字段，手机号必填；角色、部门、岗位、状态都在同一个弹窗内维护。
- 独立“角色/分配医院角色”按钮、弹窗和前端 `/roles` 调用已移除，角色变更统一随新增/修改保存。
- 当前仍依赖后端按当前医院上下文和当前用户角色层级过滤同级/上级医院用户，前端只保留 owner 行禁用修改/删除的交互保护。
...
## 2026-06-27 任务 4：默认角色与菜单默认可见角色前端调整

本次前端改动集中在系统菜单和医院角色：

- 系统菜单管理新增业务菜单“默认可见角色”多选，选项来自 `hospital_id=0` 的默认业务角色，只提交默认角色 ID，不能写成角色层级。
- 业务菜单识别按 `medical:` 权限标识或 `medical` 前端路径判断；新增业务菜单未选择默认可见角色时不允许保存。
- 编辑已有业务菜单时提示“影响仍使用默认角色的医院，不影响已替换角色的医院”。
- 医院角色列表不展示默认列；编辑已有角色时角色名称和角色层级置灰，由后端负责默认角色替换、成员边界和权限保存校验。
- 医院角色权限树继续使用后端菜单树接口，保持按当前登录身份返回可授权范围，避免前端额外暴露自己没有的权限。
