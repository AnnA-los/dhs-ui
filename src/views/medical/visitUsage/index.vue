  ...
  <div class="app-container visit-usage-page">
    ...
    <div ref="tablePanel" class="usage-table-panel">
      <el-table v-loading="loading" :data="usageList" :height="tableHeight" border fit>
        ...
        <el-table-column label="可用数量" prop="availableQuantity" width="110" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="560px" append-to-body>
      <el-form ref="form" :model="form" label-width="100px">
        <el-form-item label="明细类型"><el-radio-group v-model="form.itemType" @change="handleItemTypeChange"><el-radio label="MEDICINE">药品</el-radio><el-radio label="CONSUMABLE">耗材</el-radio><el-radio label="PROJECT">项目</el-radio><el-radio label="OTHER">其他</el-radio></el-radio-group></el-form-item>
        <el-form-item v-if="form.itemType !== 'OTHER'" label="物品">
          <el-select v-model="form.itemId" filterable remote clearable placeholder="请选择物品" :remote-method="remoteItems" :loading="itemLoading" style="width: 100%" @change="handleItemChange">
            <el-option v-for="item in itemOptions" :key="item.value" :label="item.label" :value="item.value">
              <span>{{ item.label }}</span><span v-if="item.remainingQuantity !== undefined" style="float:right;color:#909399">可用 {{ item.remainingQuantity }}</span>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item v-if="form.itemType === 'OTHER'" label="名称"><el-input v-model="form.itemName" /></el-form-item>
        <el-form-item v-if="showAvailable" label="可用数量"><el-input :value="currentAvailable" disabled /></el-form-item>
        ...
        <el-form-item label="单价"><el-input-number v-model="form.unitPrice" :disabled="form.itemType !== 'OTHER'" :precision="2" :min="0" @change="refreshTotal" /></el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleItemTypeChange() { this.form.itemId = undefined; this.form.itemName = undefined; this.form.unitPrice = 0; this.form.totalPrice = 0; this.currentAvailable = 0; this.itemOptions = []; if (this.form.itemType !== 'OTHER') this.remoteItems('') },
    remoteItems(query) {
      this.itemLoading = true
      const request = this.form.itemType === 'MEDICINE' ? medicineOptions({ medicineName: query }) : (this.form.itemType === 'CONSUMABLE' ? consumableOptions({ consumableName: query }) : projectOptions({ projectName: query, saleStatus: '0' }))
      request.then(r => {
        const rows = r.data || []
    ...
    submitForm() {
      this.refreshTotal()
      if (this.form.itemType !== 'OTHER' && !this.form.itemId) { this.$modal.msgError('请选择物品'); return }
    ...
    itemTypeName(type) { return { MEDICINE: '药品', CONSUMABLE: '耗材', PROJECT: '项目', OTHER: '其他' }[type] || type }
  }
}
</script>
<style scoped>
